...
SUGAR.tour = function() {
    ...
    return {
        init: function(params) {
            ...
            tourModal = $('<div id="' + params.id + '" class="modal ' + params.className + ' tour slides"></div>').modal({
                backdrop: false
            }).draggable({
                handle: ".modal-header"
            });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < modals.length; i++) {
                        ...
                        modalArray[i] = $('<div id="' + modalId + '" class="modal ' + params.className + ' tour slides"></div>').modal({
                            backdrop: false
                        }).draggable({
                            handle: ".modal-header"
                        });
                        ...
                        $(modals[i].target).ready(function() {
                            ...
                            $(modals[i].target).popoverext({
                                ...
                                onShow: function() {
                                    $('.pointer').css('top', '0px');
                                    $(".popover .pointer").effect("custombounce", {
                                        times: 1000,
                                        direction: bounce,
                                        distance: 50,
                                        gravity: false
...
SUGAR.image = {
    ...
    lightbox: function(image) {
        ...
        SUGAR.image.lighboxWindow.setBody("<img src='" + image + "' width='400px'> </img>");
                ...
                buttons: [{
                    ...
                    handler: function() {
                        ...
                        var callback = {
                            success: function(o) {
                                window.setTimeout(function() {
                                    if (document.getElementById('search_form')) document.getElementById('search_form').submit();
            ...
            if (r.menu && r.menu.module) {
                ...
                if (hl.length > 0) {
                    hl.find('a').each(function() {
                        this.href = this.href.replace(new RegExp("help_action=([^\&]*)"), 'help_action=' + action_sugar_grp1).replace(new RegExp("help_module=([^\&]*)"), 'help_module=' + r.menu.module);
                    });
...
SUGAR.expressions.Expression.prototype.validateParameters = function() {
    ...
    if (count == 0 && typeof(params) == 'undefined') {
        return;
    }
...
SUGAR.expressions.Expression.prototype.isProperType = function(variable, type) {
    ...
    switch (type) {
        case see.STRING_TYPE:
            return (typeof(variable) == 'string' || typeof(variable) == 'number' || variable instanceof see.TYPE_MAP[see.NUMERIC_TYPE]);
            break;
...
SUGAR.expressions.Expression.prototype.getClass = function(exp) {
    for (var i in SUGAR.FunctionMap) {
        if (typeof SUGAR.FunctionMap[i] == "function" && SUGAR.FunctionMap[i].prototype && this instanceof SUGAR.FunctionMap[i])
            return i;
    }
    return false;
};
...
SUGAR.expressions.ExpressionParser.prototype.tokenize = function(expr) {
    ...
    var justReadString = false;
    var justReadComma = false;
    var isInQuotes = false;
    var isPrevCharBK = false;
...
SUGAR.util.DateUtils = {
    parse: function(date, oldFormat) {
        ...
        var jsDate = new Date("Jan 1, 1972 00:00:00");
        var part = "";
        var dateRemain = YAHOO.lang.trim(date);
        oldFormat = YAHOO.lang.trim(oldFormat) + " ";
...
(function() {
    ...
    var attachListener = function(el, callback, scope, view) {
        ...
        } else if (el.type && el.type.toUpperCase() == "RADIO") {
    ...
    AH.addListener = function(varname, callback, scope, view) {
        ...
        AH.LISTENERS[view][varname].push({
            el: el,
            callback: callback,
            scope: scope
        });
    ...
    AH.updateListeners = function(varname, view, el) {
        ...
        for (var i = 0; i < l.length; i++) {
            ...
            if (oldEl != el) {
                ...
                AH.LISTENERS[view][varname][i] = {
                    el: el,
                    callback: l[i].callback,
                    scope: l[i].scope
                };
            }
        }
    ...
    AH.setDateTimeField = function(field, value) {
        ...
        if (mer) {
            ...
            var a = SUGAR.expressions.userPrefs.timef.indexOf("A") != -1 ? SDU.formatDate(value, true, "A") : SDU.formatDate(value, true, "a");
    ...
    AH.getRelatedField = function(link, ftype, field, view) {
        ...
        if (typeof(linkDef[ftype]) == "undefined" || (field && typeof(linkDef[ftype][field]) == "undefined") || (ftype == "related" && (linkDef.relId || currId) && linkDef.relId != currId)) {
            var params = {
                link: link,
                type: ftype
            };
    ...
    SUGAR.util.extend(SUGAR.forms.FormExpressionContext, SUGAR.expressions.ExpressionContext, {
        getValue: function(varname) {
            var SE = SUGAR.expressions,
                toConst = SE.ExpressionParser.prototype.toConstant;
            var value = "";
    ...
    SUGAR.forms.FlashField = function(field, to_color) {
        ...
        if (typeof(to_color) == 'undefined')
            var to_color = '#FF8F8F';
        var oButtonAnim = new YAHOO.util.ColorAnim(field, {
            backgroundColor: {
                to: to_color
            }
        }, 0.2);
        ...
        if (typeof(window[tabsId]) != "undefined") {
            ...
            if (tabView.get) {
                var tabs = tabView.get("tabs");
                for (var i in tabs) {
                    if (i != tabView.get("activeIndex") && (tabs[i].get("contentEl") == parentDiv || YAHOO.util.Dom.isAncestor(tabs[i].get("contentEl"), field))) {
                        var label = tabs[i].get("labelEl");
                        if (SUGAR.forms.flashInProgress[label.parentNode.id])
                            return;
                        var tabAnim = new YAHOO.util.ColorAnim(label, {
                            color: {
                                to: '#F00'
                            }
                        }, 0.2);
