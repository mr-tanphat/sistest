...
<tr>
	...
	<td scope="row" nowrap="nowrap" width='1%' >
			<label for='name_basic'> {sugar_translate label='LBL_NAME' module='J_Voucher'}
		</td>
...
<input type='text' name='{$fields.name_basic.name}' 
    ...
    value='{$value}' title=''      accesskey='9'  >
		...
		{if $fields.student_name_basic.acl > 0}
		{counter assign=index}
	{math equation="left % right"
   		  left=$index
          right=$basicMaxColumns
	...
	<td scope="row" nowrap="nowrap" width='1%' >
		...
		<label for='student_name_basic' >{sugar_translate label='LBL_STUDENT_NAME' module='J_Voucher'}</label>
    	</td>

	
	<td  nowrap="nowrap" width='1%'>
			
<input type="text" name="{$fields.student_name_basic.name}"  class="sqsEnabled"   id="{$fields.student_name_basic.name}" size="" value="{$fields.student_name_basic.value}" title='' autocomplete="off"  >
<input type="hidden"  id="{$fields.student_id_basic.name}" value="{$fields.student_id_basic.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.student_name_basic.name}"   title="{$APP.LBL_SELECT_BUTTON_TITLE}" class="button firstChild" value="{$APP.LBL_SELECT_BUTTON_LABEL}" onclick='open_popup("{$fields.student_name_basic.module}", 600, 400, "", true, false, {literal}{"call_back_function":"set_return","form_name":"search_form","field_to_name_array":{"id":"student_id_basic","name":"student_name_basic"}}{/literal}, "single", true);'>{sugar_getimage alt=$app_strings.LBL_ID_FF_SELECT name="id-ff-select" ext=".png" other_attributes=''}</button><button type="button" name="btn_clr_{$fields.student_name_basic.name}"   title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button lastChild" onclick="this.form.{$fields.student_name_basic.name}.value = ''; this.form.{$fields.student_id_basic.name}.value = '';" value="{$APP.LBL_CLEAR_BUTTON_LABEL}">{sugar_getimage name="id-ff-clear" alt=$app_strings.LBL_ID_FF_CLEAR ext=".png" other_attributes=''}</button>
</span>
