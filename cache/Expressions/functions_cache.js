...
SUGAR.util.extend(SUGAR.SugarDropDownExpression,SUGAR.EnumExpression,{className:"SugarDropDownExpression",evaluate:function(){var dd=this.getParameters().evaluate();var arr=SUGAR.language.get('app_list_strings',dd);var ret=[];if(arr=="undefined")return[];var keys=Object.keys(arr);var isNumIndex=true;var numberReg=/^\d+$/;for(var i in keys){if((typeof i=="string")&&keys[i]!=''&&!numberReg.test(keys[i])){isNumIndex=false;break;}}
if(isNumIndex){return keys.sort();}else{for(var i in arr){if(typeof i=="string"){ret[ret.length]=i;}}}
...
else if(keys instanceof Array&&(keys.length==0||(keys.length==1&&keys[0]==''))){var va=new SUGAR.forms.VisibilityAction(this.target,'false','');va.exec(context);}}});SUGAR.forms.SetRequiredAction=function(variable,expr,label){this.variable=variable;this.expr=expr;this.label=label;this._el_lbl=document.getElementById(this.label);this.msg=this._el_lbl.innerText;}
