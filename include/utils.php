...
function get_user_array($add_blank=true, $status="Active", $user_id='', $use_real_name=false, $user_name_filter='', $portal_filter=' AND portal_only=0 ', $from_cache = true) {
	global $locale, $sugar_config, $current_user;
     ...
     $custom_portal = " AND for_portal_only = 0 ";
    $portal_filter .= $custom_portal; 
    //end
    if (empty($locale)) {
        $locale = Localization::getObject();
    }
	...
	if(empty($user_array)) {
		...
		if (empty($status)) {
			...
			$query = "SELECT id, first_name, last_name, user_name from users WHERE portal_user <> 1 AND status='$status'".$portal_filter;
		}
