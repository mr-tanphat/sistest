...
class bc_SurveyViewReport extends SugarView {
    ...
    function display() {
        ...
        if (!$checkSurveySubscription['success']) {
            ...
            $survey = getReportData($type, $survey_id);
            if ($type == 'status') {
            ...
            $answerSubmissionCount = getAnswerSubmissionCount($survey_id);
            $answer_skipp = getAnswerSubmissionAnsweredAndSkipped($survey_id);
            ...
            if ($type == 'question') {
                ...
                foreach ($survey as $que_id => $question_name) {
                    $ratingAnsweredPerson = 0;
                    $oQuestion = new bc_survey_questions();
                    $oQuestion->retrieve($que_id);
                    if ($oQuestion->question_type != 'Image' && $oQuestion->question_type != 'Video') {
                        ...
                        $get_sub_que_query = "SELECT
                                    COUNT(sub_que.id) AS total_submitted_que
                                  FROM bc_submission_data_bc_survey_questions_c AS sub_que
                                  WHERE sub_que.bc_submission_data_bc_survey_questionsbc_survey_questions_ida = '{$que_id}' AND deleted = 0";
                        ...
                        if (in_array($oQuestion->question_type, array('CommentTextbox', 'Textbox', 'ContactInformation', 'Rating', 'Date', 'Scale', 'Matrix'))) {
                            $result = getQuestionWiseData($survey_id, $que_id, $oQuestion);
                            $scale_ans_count = 0;
                            ...
                            while ($row = $db->fetchByAssoc($result)) {
                                ...
                                if ($oQuestion->question_type == 'ContactInformation') {
                                    ...
                                    if (empty($row['name'])) {
                                        ...
                                        $rating_final_count[$row['name']] = $rating_final_count[$row['name']] + 1;
                                    }

                                    if (!empty($row['name'])) {
                                        $ratingAnsweredPerson++;
                                    }
                                    //Add answered and skipped persons of rating
                                    $AnsweredAndSkippedPerson[$que_id]['answered'] = $ratingAnsweredPerson;
                                    $AnsweredAndSkippedPerson[$que_id]['skipped'] = ($total_submitted_que - (int) $ratingAnsweredPerson);
                                    ...
                                    $answer = getAnswerSubmissionDataForMatrix($survey_id, '', $que_id);
                        ...
                        if ($oQuestion->question_type == 'Rating') {
                            ...
                            foreach ($rating_final_count as $key => $count) {
                                ...
                                if ($total_submitted_que != 0 && !is_array($count)) {
                                    $rating_pecent[$key] = ($count * 100) / $total_submitted_que;
                                }
                            }
...
//                                            $chart_display_data[$qid] = $multi_data;
                ...
                foreach ($module_types as $id => $module_detail) {
                    $name[$id] = array(
                        ...
                        'module_type' => $module_detail['module_type'],
