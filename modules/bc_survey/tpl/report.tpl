                                ...
                                {else}
                                    {foreach from=$queReportdata.answers item=answers key=ansid}
                                        ...
                                        {elseif $queReportdata.que_type eq 'CommentTextbox' || $queReportdata.que_type eq 'Textbox' || $queReportdata.que_type eq 'Date'}
                                            ...
                                            <tr>
                                            <td>
                                            <p class="respond_con">No Submission for this Question.</p>
                                            </td>
                                            </tr>
                                            {else}*}
                                            {foreach from=$answers.ans_name key=title item=answer_text}  
        ...
        <div>
            <div class="search-block">
                <input type="text" name="name" id="name"  onKeydown="Javascript: if (event.keyCode == 13)
                            getSearchResult('{$type}', '{$survey_id}',{$page}, 'search');" placeholder=" Search By Name">
                <select name="module_names" id="module_names" onKeydown="Javascript: if (event.keyCode == 13)
                            getSearchResult('{$type}', '{$survey_id}',{$page}, 'search');">
                ...
                <input type="button" name="Search" id="Search" value="SEARCH" onclick="getSearchResult('{$type}', '{$survey_id}', 1, 'search');">
                <input type="button" name="Clear" id="Clear" value="CLEAR" onclick="getSearchResult('{$type}', '{$survey_id}', 1, 'clear');">
            ...
            <div class="select-que">
                <table class="individual_report_table" id="search_result">
                    ...
                    {foreach from=$name key=module_id  item=module_detail}
                        <tr>
                            <td><a href="javascript:void(0);" onclick="getReports('{$survey_id}',{$page}, '{$module_detail.module_id}');">{$module_detail.name}</a>
                            </td>
                            ...
                            <td id="request_status">
                                ...
                                {else}
                                    <a href="javascript:void(0);" onclick="ApproveChRequest(this, '{$survey_id}', '{$module_detail.module_id}', '{$module_detail.module_type}');">{$module_detail.change_request}</a>
                                {/if}
                            </td>
                            <td id="re-send">
                                {if $module_detail.survey_status eq 'Submitted' || $module_detail.survey_status eq 'Viewed'}
                                    <a title="Re-send" onclick="reSendSurvey(this, '{$survey_id}', '{$module_detail.module_id}', '{$module_detail.module_type}');" href="javascript:void(0);"><img src="custom/include/images/re-send.png" style="height: 22px;"></a>
                                    {/if}
                            </td>
                        </tr>
                    {/foreach}
