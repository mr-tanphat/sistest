...
class bc_surveyController extends SugarController {
        ...
        if ($_FILES['logo_file']['tmp_name'] != '') {
            if (is_array($_FILES)) {
                if (is_uploaded_file($_FILES['logo_file']['tmp_name'])) {
                    ...
                    $targetPath = "custom/include/surveylogo_images/" . $surveyID;
                    unlink($targetPath);
                    if (move_uploaded_file($sourcePath, $targetPath)) {
                        $survey->logo = $surveyID;
                    }
                }
            }
        }
            ...
            foreach ($_REQUEST['que_title'][$pg_index] as $que_index => $que_title) {
                ...
                if (isset($_REQUEST['enable_scoring_dropdownlist'][$pg_index][$que_index])) {
                    ...
                    foreach ($_REQUEST['answer'][$pg_index][$que_index] as $ans_index => $answer) {
                        ...
                        if (isset($_REQUEST['score_dropdownlist'][$pg_index][$que_index][$ans_index])) {
                            $weight_option = $_REQUEST['score_dropdownlist'][$pg_index][$que_index][$ans_index];
                        } else {
                            $weight_option = 0;
                        }
                        $survey_ans->score_weight = $weight_option;
    ...
    */
    function action_getReports() {
        ...
        bc_survey_submission.id,bc_survey_submission.status,bc_survey_submission.module_name,
            ...
            if ($row['status'] == 'Pending') {
                ...
                if ($row['status'] == 'Submitted') {
                    ...
                    // Matrix type then get rows & columns value & generate selected answer layout
                        ...
                        $answers = getAnswerSubmissionDataForMatrix($survey_id, $module_id, $row['question_id']);
        ...
        if (count($survey_details) && $module_id != 'Select Record') {
            ...
            if (count($IndivisualReportData) != 0) {
                ...
                $queReoort_pageNumbers = "<div class='numbers'> " . $pagination->getLinks($_POST, $survey_id, $page, $module_id) . "</div>";
            }
        }
        ...
        if ($status != "Pending") {
            ...
            if ($page == 1) {
                if (isset($survey_send_date)) {
                    $send_date = TimeDate::getInstance()->to_display_date_time($survey_send_date);
                }
                if (isset($survey_receive)) {
                    $receive_date = TimeDate::getInstance()->to_display_date_time($survey_receive);
                }
                $html .= "<div class='send-receive-time'><div class='send-date'><label><strong>Description : </strong></label>{$description}</div>";
                ...
                if (isset($receive_date)) {
                    ...
                    $html .= "<div class='send-date'><label><strong>Survey Score : </strong></label>Obtained Score<strong> {$obtain_score} </strong>Out of<strong> {$base_score}</strong></div></div>";
                }
            }
        }
            ...
            foreach ($page_data as $que_id => $que_title) {
                ...
                foreach ($que_title as $title => $answers) {
                    ...
                    if($answers['enable_scoring']){
                        $html .= "<p class='que'><label>Question</label>{$title}<span style=' float:right;font-weight:bold;background-color: #DDDDDD; border-radius: 4px; height: 18px; padding:5px;'>{$answers['obtained_que_score']} / {$answers['base_que_score']} </span></p>";
                    }else{
                        $html .= "<p class='que'><label>Question</label>{$title}</p>";
                    }
                        ...
                        foreach ($answers as $key => $answer) {
                            if ($key != 'obtained_que_score' && $key != 'base_que_score' && $key != 'enable_scoring') {
                                if (is_array($answer)) {
                                    ...
                                    $html .= "<span class='ans'><label style='width:100px'>Answer</label><div style='margin-top: -16px;'><ul>";
                                    $html1 = '';
    ...
    */
    function action_getSearchResult() {
        ...
        $records = getReportData($report_type, $survey_id, $name, $type, $status);
        if (count($records) == 0) {
        ...
        if (count($records) > 0) {
            ...
            $html .= " <tr>
            <td style='font:14px;font-weight: bold;margin: 5px'>Name</td>
            <td style='font:14px;font-weight: bold;margin: 5px'>Type</td>
            <td style='font:14px;font-weight: bold;margin: 5px'>Status</td>
            ...
            foreach ($IndividualReportData as $module_id => $module_detail) {
                ...
                <td>{$module_detail['module_type']}</td>
                ...
                $html .="</td><td id='re-send'><a title='Re-send' href='javascript:reSendSurvey(\"{$survey_id}\",\"{$module_detail['module_id']}\",\"{$module_detail['module_type']}\");'><img src='custom/include/images/re-send.png' style='height: 22px;'></a></td></tr>
                ";
            }
    ...
    */
    function action_exportToExcel() {
        ...
        $finalExportData = getAllExportData($report_type, $survey_id, $name, $type, $status);
    ...
    */
    function action_approveRequest() {
        ...
        AND bc_survey_submission.module_name = '{$_REQUEST['module_name']}'
        AND bc_survey_submission.deleted = 0
        ...
        if ($db->query($updQry)) {
            ...
            CustomSendEmail($to_Email, $emailSubject, htmlspecialchars_decode($emailBody, ENT_QUOTES), $_REQUEST['module_name'], $_REQUEST['module_id']);
        } else {
    ...
    */
    function action_SendSurveyEmail() {
        ...
        $total_seleted_records = ($_REQUEST['total_selected'] != "0") ? $_REQUEST['total_selected'] : 1;
    ...
    function action_GetSurveys() {
        ...
        if ($query_result->num_rows > 0) {
            while ($survey = $db->fetchByAssoc($query_result)) {
                ...
                $html .= "<tr><td style='width: 10%;'>{$index}</td><td style='width: 60%;'>{$survey['title']}</td><td style='width: 20%; text-align: right;white-space: nowrap;' colspan='3'><input type='button' value='Send' onclick=\"schedule_survey('{$survey['id']}','{$current_date}');\">&nbsp;<input type='button' value='Send Later' onclick=\"schedule_survey_form(this,'{$survey['id']}');\">&nbsp;<input type='button' value='Preview' onclick=\"redirectToEmailTemplate('{$survey['id']}');\"></td></tr>";
                $index++;
            }
        } else {
            ...
            foreach ($template->bc_survey_pages_bc_survey_template->getBeans() as $pages) {
                ...
                foreach ($pages->bc_survey_pages_bc_survey_questions->getBeans() as $survey_questions) {
                    ...
                    foreach ($survey_questions->bc_survey_answers_bc_survey_questions->getBeans() as $survey_answers) {
                        $questions[$survey_questions->question_sequence]['answers'][$survey_answers->answer_sequence] = array('id' => '', 'name' => $survey_answers->name, 'and_id_del' => '', 'score_value' => $survey_answers->score_weight);
                    }
        ...
        if (isset($record_id) && $record_id != '') {
            ...
            foreach ($survey->bc_survey_pages_bc_survey->getBeans() as $pages) {
                ...
                foreach ($pages->bc_survey_pages_bc_survey_questions->getBeans() as $survey_questions) {
                    ...
                    $survey_questions->load_relationship('bc_survey_answers_bc_survey_questions');
                    foreach ($survey_questions->bc_survey_answers_bc_survey_questions->getBeans() as $survey_answers) {
                        $questions[$survey_questions->question_sequence]['answers'][$survey_answers->answer_sequence] = array('id' => $survey_answers->id, 'name' => $survey_answers->name, 'and_id_del' => $survey_answers->id, 'score_value' => $survey_answers->score_weight, 'skip_action' => $survey_answers->logic_action, 'skip_target' => $survey_answers->logic_target);
                    }
                    if (isset($questions[$survey_questions->question_sequence]['answers']) && is_array($questions[$survey_questions->question_sequence]['answers']))
                        ksort($questions[$survey_questions->question_sequence]['answers']);
                }
