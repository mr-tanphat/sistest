...
if(ACLController::checkAccess('J_Payment', 'edit', true) && ($GLOBALS['current_user']->team_type == 'Junior'))
    $module_menu[]=Array("index.php?module=J_Payment&action=EditView&return_module=J_Payment&return_action=DetailView&payment_type=Transfer%20From%20AIMS", $mod_strings['LNK_NEW_TRANSFER_FROM_AIMS']);

if(ACLController::checkAccess('J_Payment', 'edit', true))
    $module_menu[]=Array("index.php?module=J_Payment&action=EditView&return_module=J_Payment&return_action=DetailView&payment_type=Refund", $mod_strings['LNK_NEW_REFUND']);
