<?php
    function getJuniorCenter($center_id) {
        ...
        FROM teams t1
        LEFT JOIN teams t2 ON t1.id = t2.parent_id AND t2.deleted = 0
        WHERE t2.id IS NULL AND t1.deleted = 0 AND t1.team_type = '{$current_user->team_type}'
        #AND length(t1.sms_config) > 30
        $qr
        ORDER BY t1.name
        ";
