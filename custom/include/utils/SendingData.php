...
function checkDuplication($param){
    ...
    if(!empty($birthdate)){
        $sql = "SELECT t.name
        FROM leads l INNER JOIN teams t ON t.id = l.team_id
        AND l.deleted = 0
        AND l.full_lead_name = '$name'
        AND l.birthdate = '$birthdate'
        AND l.phone_mobile = '$phone_number' ";
        $sql_contacts = "SELECT t.name
        FROM contacts l INNER JOIN teams t ON t.id = l.team_id
        AND l.deleted = 0
        AND l.full_student_name = '$name'
        AND l.birthdate = '$birthdate'
        AND l.phone_mobile = '$phone_number'";
    } else{
    ...
    $contact_rs = $GLOBALS['db']->getOne($sql_contacts);
    if(!empty($rs) || !empty($contact_rs)){
        if(!empty($rs)){
            $team_name = $rs;    
        }else    $team_name = $contact_rs;
        return array(
            "success" => "0",
