...
//html for multi choice question type
    if (multi_choice_que == true) {
        ...
        data += "<div align='right'><a id='que_remove_" + que_no + "' style='cursor:pointer;' onclick='removeQuestions(this," + page_no + ")'><i class='fa fa-remove' id='remove_que_" + que_no + "' title='Remove Question' style='font-size:14px;margin: 5px 3px;'></i></a></div>";
        data += "<div id='tabs" + page_no + que_no + "'>";
        data += "<div class='tab' style='margin-bottom: 20px;'>"
        data += '   <a class="advance_tab active" id="gen" onclick="change_tab(this,' + page_no + ',' + que_no + ')"><p class="gen">General</p></a>';
        ...
        data += '</div>';
        data += "<div id='general" + page_no + que_no + "' class='tabcontent' style='display:block;'><ul class='question-section' id='question_table" + page_no + que_no + "'>";
...
//html for single choice question type
    ...
    if (queType == "Video") {
        ...
        data += "<li class='s-title'> <label class='left-label'>Video URL<span class='required'>*</span></label><span><input placeholder='Enter URL For Video' id='que_title" + page_no + que_no + "' class='input-text que_title' type='text' name='que_title[" + page_no + "][" + que_no + "]'/></span><span class='survey-logo-selection'><img style='margin-left:10px' title='i.e. http://www.youtube.com/embed/L3_gx6Fx_b0' src='themes/Sugar5/images/helpInline.gif' alt='Information' class='inlineHelpTip' border='0'></span></li>";
        data += "</div>";
        data += "<div id='advanced" + page_no + que_no + "' class='tabcontent' style='display:none'>";
    ...
    if (queType == "Date") {
        ...
        data += "<li class='s-start'><label class='left-label'>Start Date</label><input class='input-text sdate' id='stdate" + page_no + que_no + "' style='width: 10%;' placeholder='Start Date' type='text' name=start_date_time[" + page_no + "][" + que_no + "]>&nbsp;<img id='startdate" + page_no + que_no + "' style='position: relative;top: 4px;padding-left: 4px;' src='themes/Sugar5/images/jscalendar.gif'/> &nbsp; - &nbsp;<input class ='input-text edate' id='edate" + page_no + que_no + "' placeholder='End Date' style='width: 10%;' type='text' name=end_date_time[" + page_no + "][" + que_no + "]>&nbsp;<img style='position: relative;top: 4px;padding-left: 4px;' src='themes/Sugar5/images/jscalendar.gif' id='enddate" + page_no + que_no + "''/> ";
        data += "</ul></div>";
        data += "</div>";
        data += "</div>";
    }
...
function conver_quetype(el, page_no, que_no, title, help, ans, qn_id, ans_id) {
    ...
    if (data_que != undefined) {
        $('#que_' + page_no + que_no).html(data_que);
        if (title != "") {
        ...
        if (ans_detail.length >= 1) {
            ...
            var ans_idex = 0;
            var logic_detail = '';
//            $.each($('#answer_div' + page_no + que_no).find('#ans_id'), function (indx) {
//                logic_tr += "<tr class='opid_" + $(this).val() + "' id='con_value" + page_no + que_no + indx + "'><td class='logic-table-first-column'><label style='color:#000'>" + $(this).parent().find('input[type=text]').val() + "</label></td><td class='condition'><span><select style='width: auto;cursor:pointer;' class='clear_all" + page_no + que_no + "' id='skipp_logic_function" + page_no + que_no + indx + "' onchange='skipp_logic(this," + page_no + "," + que_no + "," + indx + ")' name='option_value[" + page_no + "][" + que_no + "][" + indx + "]'><option value='no_logic'>No Logic</option><option value='redirect_page'>Redirect To Page</option><option value='end_page'>End Of Survey</option><option value='redirect_url'>Redirect to Url</option><option value='show_hide_question'>Show/Hide Questions</option></select></td><td></td><td style='color:#000;'><a id='clear_" + page_no + que_no + indx + "' onclick='clear_element(this," + page_no + "," + que_no + "," + indx + ")' style='cursor:pointer;'>Clear</a></td></tr>";
//            });
//            $("#logic_table" + page_no + que_no).append(logic_tr);
//            $.each(target_value_obj, function (key, value) {
//                $.each(value, function (action, target) {
//                    logic_detail += '<input type="hidden" class="action_clear_all' + page_no + que_no + ' ansid_' + key + '" id="action' + page_no + que_no + ans_idex + '" value="' + action + '">';
//                    logic_detail += '<input type="hidden" class="target_clear_all' + page_no + que_no + ' ansid_' + key + '" id="target' + page_no + que_no + ans_idex + '" value="' + target + '">';
//                    ans_idex += 1;
//                });
//            });
//            $('#answer_div' + page_no + que_no).parent().append(logic_detail);
            $.each(target_value_obj, function (key, value) {
                $.each(value, function (action, target) {
                    var logic_tr = '';
                    ...
                    logic_tr += " <td class='logic-table-first-column'><label style='color:#000;'>hello</label></td><td class='condition'><select style='width: auto;cursor:pointer;' class='clear_all" + page_no + que_no + "' id='skipp_logic_function" + page_no + que_no + ans_idex + "' onchange='skipp_logic(this," + page_no + "," + que_no + "," + ans_idex + ")' name='option_value[" + page_no + "][" + que_no + "][" + ans_idex + "]'><option value='no_logic'>No Logic</option>";
...
function editQuestionHtml(queValue, page_no, que_no) {
    ...
    if ("CommentTextbox" === queValue.que_type) {
        que_icon = "<i class='fa fa-comments-o' aria-hidden='true'></i>";
        single_choice_que_type = "CommentTextbox";
        single_choice_que = true;
    }
    if ("ContactInformation" === queValue.que_type) {
        que_icon = "<i class='fa fa-list-alt' aria-hidden='true'></i>";
        single_choice_que_type = "ContactInformation";
        single_choice_que = true;
    }
    if ("Rating" === queValue.que_type) {
        que_icon = "<i class='fa fa-star' aria-hidden='true'></i>";
        single_choice_que_type = "Rating";
        single_choice_que = true;
    }
//html for single choice question type
    if (single_choice_que == true) {
        ...
        data += "<input type='hidden' name='que_id_del[" + page_no + "][" + que_no + "]' value='' id='que_id_del" + page_no + que_no + "'>";
        data += "<li class='s-type'><label class='left-label'>Question Type </label><span style='cursor:pointer;' class='change_request' id='change_" + page_no + que_no + "' ondblclick='drop_down(this," + page_no + "," + que_no + ")'>" + que_icon + single_choice_type_value[single_choice_que_type] + "&nbsp;&nbsp;<a class='fa fa-pencil' aria-hidden='true'></a></span><span><input type='hidden' name='que_type[" + page_no + "][" + que_no + "]' id='que_type" + page_no + "" + que_no + "' value='" + single_choice_que_type + "' /></span>  </li>";
        data += "<li class='s-title'> <label class='left-label'>Question<span class ='required'>*</span></label><span><input class='input-text que_title' type='text' name='que_title[" + page_no + "][" + que_no + "]' value='" + queValue.que_title + "' /></span></li>";
        data += "</div>";
        if (single_choice_que_type == "Textbox") {
            ...
            if (queValue.advance_type == "Integer") {
                data += '<li class="data_type"><label class="left-label">DataType</label><select id="data_type' + page_no + que_no + '" class="datatype-textbox" onchange="textbox_dropdown(this,' + page_no + ',' + que_no + ')" name="datatype_textbox[' + page_no + '][' + que_no + ']"><option value="0">Select</option>';
        ...
        if (single_choice_que_type == "CommentTextbox") {
            data += "<div id='advanced" + page_no + que_no + "' class='tabcontent' style='display:none'>";
        ...
        if (single_choice_que_type == "ContactInformation") {
            data += "<div id='advanced" + page_no + que_no + "' class='tabcontent' style='display:none'>";
            data += "<ul><li class='s-title'><label class='left-label'>Help Tips </label><span><input  class='input-text question_help_comment' placeholder='Enter help tips for question' type='text' name='question_help_comment[" + page_no + "][" + que_no + "]' value='" + queValue.question_help_comment + "'></span></li>";
            if (queValue.is_required === "1") {
        ...
        if (single_choice_que_type == "Rating") {
            data += "<div id='advanced" + page_no + que_no + "' class='tabcontent' style='display:none'>";
            data += "<ul><li class='s-title'><label class='left-label'>Help Tips </label><span><input class='input-text question_help_comment' placeholder='Enter help tips for question' type='text' name='question_help_comment[" + page_no + "][" + que_no + "]' value='" + queValue.question_help_comment + "'></span></li>";
            if (queValue.is_required === "1") {
                data += "<li class='s-required'><label class='left-label'>Is required  </label><span><input type='checkbox' name='is_required[" + page_no + "][" + que_no + "]' id='is_required' checked/></span></li>";
    ...
    if ("Checkbox" === queValue.que_type) {
        que_icon = "<i class='fa fa-check-square-o' aria-hidden='true'></i>";
        multi_choice_que_type = "Checkbox";
        multi_choice_que = true;
    }
    if ("DrodownList" === queValue.que_type) {
        que_icon = "<i class='fa fa-chevron-down' aria-hidden='true'></i>";
        multi_choice_que_type = "DrodownList";
        multi_choice_que = true;
    }
    if ("RadioButton" === queValue.que_type) {
        que_icon = "<i class='fa fa-dot-circle-o' aria-hidden='true'></i>";
        multi_choice_que_type = "RadioButton";
        multi_choice_que = true;
    }
    ...
    if (multi_choice_que == true) {
        ...
        if ($('#EditView input[name=module]').val() != "bc_survey_template") {
            if ($('input[name=record]').val() != '') {
                ...
                data += '   <a class="advance_tab disabled" id="lgc" onclick="change_tab(this,' + page_no + ',' + que_no + ')" ><p class="lgc">Logic</p></a>';
            }
        }
        data += '</div>';
        data += "<div id='general" + page_no + que_no + "' class='tabcontent' style='display:block;'><ul class='question-section' id='question_table" + page_no + que_no + "'>";
        ...
        var length = Object.keys(queValue.answers).length;
        var custom_len = 1;
        var showPlusBtn = 'display:none';
        var showremBtn = 'display:none';
        if (queValue.enable_scoring == 1) {
        ...
        if ($('#EditView input[name=module]').val() != "bc_survey_template") {
            if ($('input[name=record]').val() != '') {
                ...
                $.each(queValue.answers, function (ansindex, answer) {
                    ...
                    data += "<tr class='opid_" + answer.id + "' id='con_value" + page_no + que_no + (ansindex - 1) + "'><td class='logic-table-first-column'><label style='color:#000;'>" + answer.name + "</label></td><td class='condition'><span><select style='width: auto;cursor:pointer;' class='clear_all" + page_no + que_no + "' id='skipp_logic_function" + page_no + que_no + (ansindex - 1) + "' onchange='skipp_logic(this," + page_no + "," + que_no + "," + (ansindex - 1) + ")' name='option_value[" + page_no + "][" + que_no + "][" + (ansindex - 1) + "]'><option value='no_logic'>No Logic</option>";
...
//image question edit.  
    ...
    if (queValue.que_type == "Video") {
        ...
        data += "<input type='hidden' name='que_id_del[" + page_no + "][" + que_no + "]'  id='que_id_del" + page_no + que_no + "'>";
        data += "<li class='s-type'><label class='left-label'>Question Type  </label><span style='cursor:pointer;' class='change_request' id='change_" + page_no + que_no + "' ondblclick='drop_down(this," + page_no + "," + que_no + ")'>" + que_icon + " Video&nbsp;&nbsp;<a class='fa fa-pencil' aria-hidden='true'></a></span><span><input type= 'hidden' name='que_type[" + page_no + "][" + que_no + "]' id='que_type" + page_no + "" + que_no + "' value='Video' /></span>  </li>";
        data += "<li class='s-title'> <label class='left-label'>Video URL<span class='required'>*</span></label><span><input placeholder='Enter URL For Video' class='input-text que_title' type='text' name='que_title[" + page_no + "][" + que_no + "]' value='" + queValue.que_title + "'/></span><span class='survey-logo-selection'><img style='margin-left:10px' title='i.e. http://www.youtube.com/embed/L3_gx6Fx_b0' src='themes/Sugar5/images/helpInline.gif' alt='Information' class='inlineHelpTip' border='0'></span></li>";
        data += "</div>";
        data += "<div id='advanced" + page_no + que_no + "' class='tabcontent' style='display:none'>";
...
function removeQuestions(event, page_no) {
    ...
    var confirm_value = confirm("Are you sure want to remove this question ?");
    if (confirm_value) {
        if ($("#que_deleted").val()) {
...
function addNewPage(page) {
    ...
    new_page_html += "<div class='add-que append_question' id='page_" + page + "'><div class='add_question'>To add a question, simply drag it from the Page Component.</div></div>"
    new_page_html += "</div>";
                ...
                $.each(detaildata, function (page_indx, page) {
                    if (show == true) {
                        ...
                        if (page_indx != 'module') {
                            ...
                            loadQuestionDetailView(page_indx, page.page_questions);
                        }
                    }
                });
...
function loadQuestionDetailView(page_indx, que) {
    var question_detail = '';
    var required = "";
    var comment = "";
 ...
 */
$(function () {
    var ac_detail = getAttrValFromUrl("action")
    loadPagesDetailView();
    //pages become sortable
    $('div#right-nav').sortable({
        cancel: ".add-pages,h4,select,textarea,input,i,.row,.add-que",
        axis: "y",
    ...
    if (action == "edit_template") {
        ...
        record_module = {record: record_id, template_id: template_id};
    }
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                $.each(jsonToString, function (pageIndex, pageValue) {
                    if (pageIndex != "name" && pageIndex != "description" && pageIndex != "theme") {
                        new_page_html = editNewPage(pageValue);
                ...
                $('.add-page-title').droppable({
                    ...
                    cancel: ".add-pages,h4,input,textarea,select,i,.row,.add-que",
                    placeholder: 'placeholder-question',
                    items: "> div:not(.row,h4,textarea,input,i,.add-que)",
                    axis: "y",
                    cursor: "move"
                ...
                $(".add-pages").droppable({
                    ...
                    drop: function (e, ui) {
                        ...
                        $('.add-page-title').droppable({
                            ...
                            cancel: ".add-pages,h4,textarea,select,input,i,.row,.add-que",
                            placeholder: 'placeholder-question',
                            items: "> div:not(.row,h4,textarea,input,i,.add-que)",
                            axis: "y",
                            cursor: "move"
                    ...
                    cancel: ".add-pages,select,textarea,h4,input,i,.row,.add-que",
                    placeholder: "placeholder-pages",
                    axis: "y",
                    cursor: "move",
                    sort: function (event, ui) {
...
//getting value for last page no from hidden field 
            ...
            //defalt page add
            $(".add-pages").before(addNewPage(page));
        }
    }
...
//make droppable for first page 
    $('.add-page-title').droppable({
        ...
        cancel: ".add-pages,h4,select,textarea,input,i,.row,.add-que",
        placeholder: 'placeholder-question',
        items: "> div:not(.row,h4,input,textarea,i,.add-que)",
        axis: "y",
        cursor: "move"
    });
    //dragging new page
    ...
    $('#plus-image').click(function () {
        ...
        var page_html = addNewPage(page_no + 1);
        ...
        if (record_module.record == '' && record_module.template_id == 'null') {
            ...
            $('.add-page-title').droppable({
                ...
                cancel: ".add-pages,textarea,select,h4,input,i,.row,.add-que",
                placeholder: 'placeholder-question',
                items: "> div:not(.row,h4,input,textarea,i,.add-que)",
                axis: "y",
                cursor: "move"
