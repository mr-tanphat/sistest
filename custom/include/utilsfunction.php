 ...
 */
function getReportData($type, $survey_id, $name = '', $module_type = '', $status = '') {
    global $db;
    if ($type == 'status') {
        $query = "SELECT
        ...
        $get_question_query = "SELECT
                    questions.name,questions.id,bc_survey_pages.page_sequence as page_seq,bc_survey_pages.name as pagename
        ...
        $query = "SELECT submission.module_name AS module_type,
                    ...
                    IF(submission.status = 'Submitted', submission.date_modified,'N/A') AS receive_date,
        ...
        if (!empty($module_type)) {
            $type_where = " AND submission.module_name = '{$module_type}'";
        }
        if (!empty($status)) {
            if ($status == 'Pending') {
 ...
 */
function sendSurveyEmailsModuleRecords($records, $module_name, $survey_id, $schedule_on_date = '', $schedule_on_time = '', $isSendNow = false, $beanFromAutomizer, $toemail) {
    ...
    foreach ($recipients as $rec_module => $rec_module_ids) {
        foreach ($rec_module_ids as $rec_module_id) {
            ...
            if ($focus->email_opt_out == 0) {
                ...
                $survey_submission->module_name = $rec_module;
    ...
    while ($row = $db->fetchByAssoc($result)) {
        ...
        if ($row['status'] == 'Pending') {
            ...
            if ($row['status'] == 'Submitted') {
                ...
                elseif (!empty($row['question_type']) && $row['question_type'] != 'Image' && $row['question_type'] != 'Video') {
 ...
 * @return     array
 */
function getPerson_SubmissionExportData($survey_id, $module_id, $export) {
    global $db, $sugar_config;
    $get_result_query = " SELECT
                          questions.name AS question_title,
                          questions.id           AS question_id,
    ...
    while ($row = $db->fetchByAssoc($result)) {
        ...
        // Matrix type then get rows & columns value & generate selected answer layout
            ...
            $detail_array[$row['question_id']][$row['question_title']]['answer_detail'] = $answer;
            $i++;
        } else if (!empty($row['question_type']) && ($row['question_type'] == 'Checkbox' || $row['question_type'] == 'MultiSelectList') && !($export)) {
            if (array_key_exists($row['question_title'], $detail_array)) {
            ...
            if (array_key_exists($row['question_title'], $detail_array)) {
                $detail_array[$row['question_title']] = $detail_array[$row['question_title']] . ',' . $row['answer_name'];
            } else {
                $detail_array[$row['question_title']] = $row['answer_name'];
            }
        } else {
 ...
 * @return     array
 */
function getAllExportData($type, $survey_id, $name = '', $module_type = '', $status = '') {
    $submtData = array();
    $submtData = getReportData($type, $survey_id, $name, $module_type, $status);
    foreach ($submtData as $sbmtId => $sbmtData) {
        $questionAnsArray = array();
        $sbmtSurvData = getPerson_SubmissionExportData($survey_id, $sbmtData['module_id'], true);
        foreach ($sbmtSurvData as $queAns) {
            $question = array_keys($queAns);
            $questionAnsArray[] = array($question[0] => $queAns[$question[0]]);
        }
    ...
    $getSubmissionId = "SELECT 
                    ...
                    WHERE
                        bc_survey.id = '{$survey_id}'
                            ...
                            AND bc_survey_submission.module_name = '{$module_name}'";
 ...
 * @return     array
 */
function getAnswerSubmissionCount($surveyID) {
    global $db;
 ...
 * @return     array
 */
function getAnswerSubmissionDataForMatrix($surveyID, $moduleID, $questionID) {
    global $db;
    if (!empty($moduleID)) {
        $selectEachAnswerSubCount = "SELECT 
                                bc_survey_submit_answer_calculation.question_id AS queId,
 ...
 * @return     array
 */
function getQuestionWiseData($survey_id, $que_id, $question_obj) {
    global $db;
...
FROM bc_survey_questions
  JOIN bc_survey_bc_survey_questions_c
    ON bc_survey_questions.id = bc_survey_bc_survey_questions_c.bc_survey_bc_survey_questionsbc_survey_questions_idb
      ...
      GROUP BY submission_data.sub_ans_id";
    $result = $db->query($query);
    return $result;
}
 ...
 * @return     array
 */
function getLineChart($type, $survey_id) {
    global $db;
    if ($survey_id) {
        $query = "SELECT
                        bc_survey_submission.status,bc_survey.name,date(bc_survey.start_date) AS start_date,
                        bc_survey.id,bc_survey_submission.email_opened ,bc_survey_submission.id AS sub_id,
                        date(bc_survey_submission.date_entered) as send_date,
                        IF(bc_survey_submission.status = 'Submitted', bc_survey_submission.date_modified,'N/A') AS receive_date,
                         date(bc_survey.end_date), date(bc_survey.start_date) 
                  FROM bc_survey_submission
                        JOIN bc_survey_submission_bc_survey_c
                          ON bc_survey_submission.id = bc_survey_submission_bc_survey_c.bc_survey_submission_bc_surveybc_survey_submission_idb
                        JOIN bc_survey
        ...
        while ($status_row = $db->fetchByAssoc($status_result)) {
            ...
            if ($status_row['receive_date'] == 'N/A') {
                $status_row['receive_date'] = $status_row['start_date'];
            }
            $date = TimeDate::getInstance()->to_display_date($status_row['receive_date']);
 ...
 * @return array
 */
function getAnswerSubmissionAnsweredAndSkipped($surveyID) {
    global $db;
    $selectEachAnswerSubCount = "SELECT 
    ...
    $GLOBALS['log']->fatal("This is the submit answer array : " . print_r($rec_array, 1));
