    ...
    if (!$checkSurveySubscription['success']) {
        ...
        $scDataQry = "SELECT
                    ...
                    WHERE submission.schedule_on <= '{$date}'
                        AND submission.survey_send = 0
                        LIMIT 150";
        $scDataQryRes = $db->query($scDataQry);
        $cnt = 0;
        while ($scDataQryResult = $db->fetchByAssoc($scDataQryRes)) {
            $recipients[$scDataQryResult['module_name']][$cnt]['module_id'] = $scDataQryResult['module_id'];
            $recipients[$scDataQryResult['module_name']][$cnt]['survey_id'] = $scDataQryResult['survey_id'];
            $recipients[$scDataQryResult['module_name']][$cnt]['submission_id'] = $scDataQryResult['submission_id'];
            $recipients[$scDataQryResult['module_name']][$cnt]['recipient_email_field'] = $scDataQryResult['recipient_email_field'];
            $cnt++;
        }
