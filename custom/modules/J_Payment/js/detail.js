...
$( document ).ready(function() {
    ...
    $('#btn_submit_convert').live('click',function(){
        if($('#cp_tuition_hours').val() == 0 || $('#cp_remain_hours').val() == 0){
        ...
        $.ajax({
            ...
            data:
            {
                ...
                convert_to_type : $("#cp_convert_to_type").val(),
            },
            dataType: "json",
    ...
    $('#add_to_class').live('click',function(){
        ...
        if( (pay_status == 'Closed' || today_date > finish_study || is_paid == '0') && is_admin != '1' ){
            alertify.alert("<span class=\"ui-icon ui-icon-alert\" style=\"float:left;\"></span> Payment này đã hết hạn hoặc chưa thanh toán đủ, hãy kiểm tra lại !!");
            return ;
        }
        ...
        if (e) {
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if (data.success == "1") {
                        //                        showSubPanel('payment_paymentdetails', null, true);
                        alertify.success('Success! Waiting for reload!');
                        location.reload();
...
function undoPayment(){
    ...
    if (paymentType == "Transfer Out" || paymentType == "Transfer In"){
        ...
        var actionReturn = 'Transfer%20Out';
    }
