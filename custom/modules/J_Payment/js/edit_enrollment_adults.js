...
function calDiscount(){
    ...
    //    //Caculated Discount Day
    //    var price_dis           = parseFloat($("#coursefee option:selected").attr('price')) / parseInt($("#coursefee option:selected").attr('type'));
    //    $('.dis_check:checked').each(function(index, brand){
    //        var dis_days = $(this).attr('discount_days');
    //        if(dis_days != '' && dis_days != null && !(typeof dis_days === "undefined")){
    //            var discount_days       = parseInt(dis_days) *price_dis;
    //            dis_discount_amount     += discount_days;
    //            $(this).closest('tr').find("td:eq(3)").text(discount_days  == "0"? ""  : Numeric.toInt(discount_days));
    //        }
    //    });

    dis_total_discount_amount     = dis_discount_amount * ratio;
    dis_total_discount_percent     = (dis_discount_percent/100) * (dis_amount_bef_discount);
    ...
    var limited_discount    = (limit_percent/100) * (dis_amount_bef_discount);    //limit discount
