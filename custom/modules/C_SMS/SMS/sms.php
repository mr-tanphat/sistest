...
class sms implements sms_interface {
    ...
    function createSMSRecord($phone_number, $message, $status, $parent_type, $parent_id, $user_id, $template_id = "", $date_in_content = "", $team_id = ""){
        ...
        $c_sms->message_count       = ceil(strlen($message)/160);
        $c_sms->template_id         = $template_id;
        $c_sms->date_in_content 	= $date_in_content != ""? $date_in_content : reset(explode(" ",$timedate->now()));
        $c_sms->assigned_user_id 	= $user_id;
        $c_sms->team_id 			= $team_id != ""? $team_id : $receiver->team_id;
        $c_sms->team_set_id 		= $team_id != ""? $team_id : $receiver->team_id;
        $c_sms->save();
    }
}
?>
