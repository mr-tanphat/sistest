...
function getSubSituation(){
    ...
    AND j_studentsituations.description <> 'Auto-Generate when changed schedule.'
    GROUP BY (CASE
    ...
    while($r = $GLOBALS['db']->fetchByAssoc($rs1)){
        ...
        if($student_id != $r['student_id']){
            ...
            if($r['no_increasement'] != $num)
                $u_num = ", no_increasement=$num";
        }else{
            if($r['no_increasement'] != 0)
                $u_num = ", no_increasement=0";
        }
        ...
        $u_status =", status='$status'";


        $GLOBALS['db']->query("UPDATE j_studentsituations SET deleted=0 $u_num $u_status WHERE id='{$r['id']}'");
