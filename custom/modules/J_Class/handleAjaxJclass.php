...
switch ($_POST['type']) {
    ...
    case 'ajaxSaveAttendance':
        $result = saveAttendance($_POST['session_id'],$_POST['student_id'],$_POST['attendance'],$_POST['in_class'],$_POST['description'],$_POST['attend_id']);
        echo $result;
        break;
...
function ajaxMakeJsonSession($class_id, $change_from, $change_to, $class_case, $start_date, $schedule, $total_hours, $change_reason, $team_type){
    ...
    foreach($schedule as $weekdate => $schl){
        $schedule_formated[$weekdate]['start_time']     = $timedate->to_db_time($schl['start_time'],false);
        $schedule_formated[$weekdate]['end_time']       = $timedate->to_db_time($schl['end_time'],false);
        $_duration = unformat_number($schl['duration_hour']);
        $schedule_formated[$weekdate]['revenue_hour']   = unformat_number($schl['revenue_hour']);
        $schedule_formated[$weekdate]['duration_hour']  = $_duration;
        $schedule_formated[$weekdate]['teaching_hour']  = unformat_number($schl['teaching_hour']);
        ...
        while($total_hours > 0){
            ...
            if (array_key_exists($chck_day, $schedule)){
                if (array_key_exists($run_date, $holiday_list)){
                    ...
                    $revenue_hour   = unformat_number($schedule[$chck_day]['revenue_hour']);
                    $teacher_hour   = unformat_number($schedule[$chck_day]['teaching_hour']);
                    $duration       = unformat_number($schedule[$chck_day]['duration_hour']);
                    ...
                    $start_time = $timedate->to_db($timedate->to_display_date($run_date,false).' '.$schedule[$chck_day]['start_time']);
                    $minutes    = $duration * 60;
                    ...
                    $sessions[$chck_day][$run_date] = $ss_date;
                    $count_sessions++;
    ...
    elseif($class_case == 'change_schedule'){
        ...
        foreach($date_in_range as $key=>$date){
            if(!array_key_exists($date, $holiday_list) && $total_remain > 0){
                $arr_schedule = $schedule[date('D',strtotime($date))];
                $revenue_hour = unformat_number($arr_schedule['revenue_hour']);
                $teacher_hour = unformat_number($arr_schedule['teaching_hour']);
                $duration     = unformat_number($arr_schedule['duration_hour']);
                //Kiểm tra trường hợp đặc biệt
                ...
                $start_time = $timedate->to_db($timedate->to_display_date($date,false).' '.$arr_schedule['start_time']);
                $minutes    = $duration * 60;
                $end_time   = date('Y-m-d H:i:s',strtotime("+$minutes minutes $start_time"));

                $lesson++;
                $sessions[$date]['lesson']      = $lesson;
                $sessions[$date]['date']        = $date;
                $sessions[$date]['week_date']   = date('D',strtotime($date));
                $sessions[$date]['start_time']  = $start_time;
                $sessions[$date]['end_time']    = $end_time;
                $sessions[$date]['revenue_hour']    = $revenue_hour;
                $sessions[$date]['teaching_hour']   = $teacher_hour;
                $run_date = $date;
            }
        }
        ...
        while($total_remain > 0){
            ...
            $date_of_last = $last_session['date'];
            $weekdays_of_last = date('D',strtotime($date_of_last));
            //neu session tạo đến khi kết thúc thì làm tiếp thứ đó
            if((strtotime($change_to_db) == strtotime($class_end_db)) &&  (array_key_exists($weekdays_of_last, $schedule))){
                $look = true;
                $date_run = strtotime($date_of_last);
                while ($look){
                    $date_run = strtotime('+1 day', $date_run);
                    if(array_key_exists(date('D',$date_run), $schedule) && !array_key_exists(date('Y-m-d',$date_run), $holiday_list)){
                        $look = false;
                        $new_date = date('Y-m-d', $date_run);
                        $arr_schedule = $schedule[date('D',$date_run)];

                        $revenue_hour = unformat_number($arr_schedule['revenue_hour']);
                        $teacher_hour = unformat_number($arr_schedule['teaching_hour']);
                        $total_remain = $total_remain - $revenue_hour;

                        $start_time = $timedate->to_db($timedate->to_display_date($new_date,false).' '.$arr_schedule['start_time']);
                        $end_time   = $timedate->to_db($timedate->to_display_date($new_date,false).' '.$arr_schedule['end_time']);
                        if($total_remain<0){
                            $revenue_hour = $revenue_hour - abs($total_remain);
                            $teacher_hour = $teacher_hour - abs($total_remain);
                            $minutes = $revenue_hour * 60;
                        ...
                        $lesson++;
                        $sessions[$new_date]['lesson'] = $lesson;
                        $sessions[$new_date]['date']   = $new_date;
                        $sessions[$new_date]['week_date']   = date('D',strtotime($new_date));
                        $sessions[$new_date]['start_time']  = $start_time;
                        $sessions[$new_date]['end_time']    = $end_time;
                        $sessions[$new_date]['revenue_hour']    = $revenue_hour;
                        $sessions[$new_date]['teaching_hour']   = $teacher_hour;
                        $run_date = $new_date;
                    }
                }
            }else{
                //Nếu tạo trong khoảng giữa thì lịch phát sinh sẽ tạo theo lịch cũ
                $look = true;
                $session_list = $json['sessions'];
                $date_run = strtotime($date_of_last);
                if(empty($session_list)) break;

                while ($look){
                    $date_run = strtotime('+1 day', $date_run);
                    if(array_key_exists(date('D',$date_run), $session_list) && !array_key_exists(date('Y-m-d',$date_run), $holiday_list)){
                        $look = false;
                        $new_date = date('Y-m-d', $date_run);
                        $first_ele = reset($session_list[date('D',$date_run)]);

                        $start_time_display = $timedate->to_display_time($first_ele['start_time']);
                        $end_time_display   = $timedate->to_display_time($first_ele['end_time']);
                        $revenue_hour = $first_ele['revenue_hour'];
                        $teacher_hour = $first_ele['teaching_hour'];
                        $total_remain = $total_remain - $revenue_hour;

                        $start_time = $timedate->to_db($timedate->to_display_date($new_date,false).' '.$start_time_display);
                        $end_time   = $timedate->to_db($timedate->to_display_date($new_date,false).' '.$end_time_display);
                        if($total_remain<0){
                            $revenue_hour = $revenue_hour - abs($total_remain);
                            $teacher_hour = $teacher_hour - abs($total_remain);
                            $minutes = $revenue_hour * 60;
        ...
        //Edit by Tung bui - 12/12/2015 - Xóa những session sau ngày end date đã tính
        ...
        if( $team_type == 'Junior'){
            ...
            foreach($situationArr as $si_id => $si_value){
                ...
                $begin_total_hour = $si_value['total_hour'];
                $till = 0;
                foreach($sessions as $ss_id => $ss_value){
                    $till += $ss_value['revenue_hour'];
                    $ss_start_till = $till - $ss_value['revenue_hour'];
                    if( ($si_value['total_hour']  > 0 ) &&  ($si_value['start_hour'] <= $ss_start_till)){
                        //Caculate First - Last Session
...
function ajaxAddOutstanding($student_id, $start, $end, $class_id, $total_hours, $add_type, $situation_id){
    ...
    $start_time_zone 	= date('Y-m-d H:i:s',strtotime("-7 hours ".$timedate->to_db_date($start	,false)." 00:00:00"));
    $end_time_zone 		= date('Y-m-d H:i:s',strtotime("-7 hours ".$timedate->to_db_date($end	,false)." 23:59:59"));

    $q1 = "SELECT DISTINCT
    IFNULL(meetings.id,'') primary_id,
    ...
    LEFT JOIN  meetings_contacts l2_1 ON meetings.id = l2_1.meeting_id AND l2_1.deleted=0 AND l2_1.contact_id='$student_id'
    LEFT JOIN  contacts l2 ON l2.id=l2_1.contact_id AND l2.deleted=0
    WHERE (((meetings.date_start >= '$start_time_zone')
    AND (meetings.date_end <= '$end_time_zone')
    AND (l1.id='$class_id')))
    ...
    $arr_ss = array();
    while($row = $GLOBALS['db']->fetchByAssoc($rs1)){
        $arr_ss[$row['primary_id']]['delivery_hour'] = $row['delivery_hour'];
        $arr_ss[$row['primary_id']]['till_hour'] = $row['till_hour'];
        if(!empty($row['student_id']))
            $count_ss_student++;
    }
    if ($count_ss_student == 0){
        ...
        foreach ($arr_ss as $key => $ss_id)
            addJunToSession($situ->id, $key);
...
function addDemo($dm_student_id, $dm_type_student, $dm_lesson_date, $dm_class_id){
    ...
    if(($_POST['action_demo'] == 'delete')){
        ...
        elseif($row['student_type'] == 'Lead'){
            $res = get_lead_in_class($row['lead_id'], $row['ju_class_id']);
            for($i = 0; $i < count($res); $i++)
                $GLOBALS['db']->query("DELETE FROM meetings_leads WHERE id = '{$res[$i]['rel_id']}'");
        }
        ...
        if($stu_delay->student_type == 'Student')
            for($i = 0; $i < count($ss); $i++)
                ...
                elseif($stu_delay->student_type == 'Lead')
                    for($i = 0; $i < count($ss); $i++){
                        $stu->load_relationship('meetings');
                        $stu->meetings->add($ss[$i]['primaryid']);
                }
...
function handleCreateDelay($situation_id, $from_date, $to_date, $dl_reason, $dl_payment_date){
    ...
    $pm_delay->delay_situation_id 				= $stu_delay->id;
...
function ajaxGetStudentList($classId,$lessonDate){
    ...
    $sqlCheckSes = "SELECT id, description FROM meetings
    ...
    if (empty($sessionId)){
        return json_encode(array(
            ...
            "content" => "<tr><td colspan='5' style='text-align:center;'>No session in {$lessonDate}</td></tr>",
        ));
    }
    //$sessionsBean = BeanFactory::getBean("Meetings", $sessionId);
    ...
    a.id atd_id,
    IFNULL(a.leaving_type, 'A') leaving_type,
    ...
    while($row = $GLOBALS['db']->fetchByAssoc($rsAllStudent)){
        ...
        if(!empty($row['session_id'])) {
            $ss->assign("IN_CLASS_TYPE", "");
        }else  $ss->assign("IN_CLASS_TYPE", "tr_not_in_class");
        ...
        $ss->assign("ATTENDANCE_CHECKED", $row['leaving_type'] == "P" ? "checked" : "");
...
function saveAttendance($sessionId, $studentId, $attendance, $inClass, $description, $attend_id){
    $attendanceBean = BeanFactory::getBean("C_Attendance", $attend_id);
    $attendanceBean->attended       = "1";
    $attendanceBean->leaving_type   = $attendance == "1"? "P" : "A";
...
function ajaxGetWeekDay($fromDate, $toDate,$classId){
    ...
    FROM
    meetings
    INNER JOIN j_class  ON meetings.ju_class_id = j_class.id
    AND j_class.deleted = 0
    WHERE
    ...
    while($row = $GLOBALS['db']->fetchByAssoc($rs)){
        $weekDate =  date('l',strtotime($row['date_end']));
        if (!in_array($weekDate, $arrayWeekDate)) $arrayWeekDate[] = $weekDate;

    }
    echo json_encode(array(
        "success" => "1",
        "array_date" => $arrayWeekDate
    ));
}
...
function ajaxGetTeacherBySchedule($classId, $from_date, $to_date, $day_of_week){
    ...
    foreach($teacherList as $key => $value){
        if ($value["priority"] == 1){
            $dayOffString = str_replace("^","",$value["day_off"]);
            ...
            $ss->assign("EXPIRE_DAY",       $timedate->to_display_date($value["contract_until"],true));
            $ss->assign("DAY_OFF",          $dayOffString);
            $ss->assign("HOLIDAYS",         $value["holiday"]);
            $ss->assign("PRIORITY_LEVEL",   "schedule_teacher_priority_1");
    ...
    foreach($teacherList as $key => $value){
        if ($value["priority"] == 2){
            ...
            $ss->assign("DAY_OFF",          $dayOffString);
            $ss->assign("HOLIDAYS",         $value["holiday"]);
            $ss->assign("PRIORITY_LEVEL",   "schedule_teacher_priority_2");
    ...
    foreach($teacherList as $key => $value){
        if ($value["priority"] == 3){
            ...
            $ss->assign("DAY_OFF",          $dayOffString);
            $ss->assign("HOLIDAYS",         $value["holiday"]);
            $ss->assign("PRIORITY_LEVEL",   "schedule_teacher_priority_3");
...
function ajaxLoadClassInfo($classId){
    ...
    $ssClass->assign("MOD", $GLOBALS['mod_strings']);
...
function handleRemoveFromClassAdult(){
    ...
    if($res_obj['studied_session'] <= 0){
        ...
        if(!empty($situa->payment_id)){
            $q2     = "SELECT id, start_study, end_study, tuition_hours FROM j_payment WHERE id='{$situa->payment_id}'";
            ...
            if(!empty($row2['start_study']) && ($row2['start_study'] == $situa->start_study) && ($date_first > $situa->start_study) && ($class_type == 'Practice')){
