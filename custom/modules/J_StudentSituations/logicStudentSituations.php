        ...
        if ($_REQUEST['module']=='J_Class' && ($_REQUEST['action']=='DetailView' || $_REQUEST['action']=='SubPanelViewer')){
            ...
            if($team_type == 'Junior'){
                ...
                if($bean->type == 'Waiting Class' && $row['class_type'] == 'Waiting Class'){
                    ...
                    $bean->custom_button .= '<input type="button"  class="button btn_add_to_class" student_name="'.$row['full_student_name'].'" situa_id="'.$bean->id.'" value="Add To Class" style="margin-right: 4px;" onclick="window.open(\'index.php?module=J_Payment&action=DetailView&record='.$row['payment_id'].'\')" />';
                }
            }


            $bean->description = '<input type="text" ss_id = "'.$bean->id.'" class = "studentsituations_description" name="description" size="25" maxlength="100" value="'.$bean->description.'" title="Note">';
