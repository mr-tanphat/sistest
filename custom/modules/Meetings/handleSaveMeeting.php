...
class handleSaveMeetings {
    function handleSaveMeetings(&$bean, $event, $arguments)
    {
        ...
        if($bean->date_modified == $bean->date_entered && $bean->meeting_type == "Session"){
            if(empty($bean->delivery_hour) && $bean->delivery_hour != 0)
                $bean->delivery_hour = $bean->duration_cal;

            if(empty($bean->teaching_hour) && $bean->teaching_hour != 0){
                $class_type = $GLOBALS['db']->getOne("SELECT type FROM c_classes WHERE id = '{$bean->class_id}'");
                if($class_type != 'Connect Club')
                    $bean->teaching_hour = $bean->duration_cal;
                else
                    $bean->teaching_hour = $bean->duration_cal;
            }
        }
        //add by Trung Nguyen 2015.12.23
        //if($bean->teacher_id != $bean->fetched_row['teacher_id']){
    ...
    function  handleSavePT(&$bean, $event, $arguments){
        ...
        if($bean->date_modified != $bean->date_entered && ($bean->meeting_type == "Placement Test" || $bean->meeting_type == "Demo")){
            ...
            elseif($bean->meeting_type == "Placement Test"){
                ...
                //                    //$dates = $GLOBALS['timedate']->to_db_date($GLOBALS['timedate']->to_display_date_time($bean->date_start));
                //                    $date = explode(' ', $bean->date_start);
                //                    $sql_get_team_name = "SELECT short_name FROM teams WHERE id = '{$bean->team_id}' and deleted = 0";
                //                    $team_name = ($GLOBALS['db']->getOne($sql_get_team_name));
                //                    $bean->name = $team_name.' DEMO'.'-'.$date[0];
...
function saveDemoPTReturnModule($meeting, $return_module, $student_id ){
    ...
    if($return_module == "Leads" || $return_module == 'Contacts'){
        ...
        if($result->parent == 'Leads') {
            $row_lead = $GLOBALS['db']->fetchOne("SELECT first_name, last_name FROM leads WHERE id='{$student_id}'");
            $result->name = $meeting->name.' - '.$row_lead['last_name'].' '.$row_lead['first_name'];
        } else {
            $sql_get_contact="SELECT full_student_name FROM contacts WHERE id='{$student_id}'";
            $result->name = $meeting->name.' - '.$GLOBALS['db']->getOne($sql_get_contact);
        }
        ...
        $meeting = BeanFactory::getBean('Meetings',$meeting->id);
        ...
        if($result->parent == 'Leads') {
            ...
            $rela_lead->leads_j_ptresult_1->add($student_id);
            //update status leads
            $sql = "UPDATE leads
            SET date_modified ='".$GLOBALS['timedate']->nowDb()."',
            working_date='".($timedate->to_db_date($meeting->date_start,false))."',
            status = 'PT/Demo'
            WHERE id='$student_id' AND status != 'Converted' ";
            $GLOBALS['db']->query($sql);
        } else {
