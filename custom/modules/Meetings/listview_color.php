...
class ListviewLogicHookMeetings {
    ...
    function listviewcolor_Meetings(&$bean, $event, $arguments) {
        ...
        elseif($bean->meeting_type == 'Session'){
            //            if($_REQUEST['module'] == 'J_Class' && $_REQUEST['action']=='DetailView'){
            //                $bean->number_of_student = $GLOBALS['db']->getOne("SELECT DISTINCT
            ...
            //                $bean->attended  = $GLOBALS['db']->getOne("SELECT DISTINCT
            //                    IFNULL(COUNT(l1.id), 0) count
            //                    FROM
            //                    meetings
            //                    INNER JOIN
            //                    c_attendance l1 ON meetings.id = l1.meeting_id
            //                    AND l1.deleted = 0
            //                    INNER JOIN
            //                    contacts l2 ON l1.student_id = l2.id AND l2.deleted = 0
            //                    WHERE
            //                    (((meetings.id = '{$bean->id}')
            //                    AND ((l1.attended LIKE 'on'
            //                    OR l1.attended = '1'))))
            //                    AND meetings.deleted = 0");
            //            }
        }
