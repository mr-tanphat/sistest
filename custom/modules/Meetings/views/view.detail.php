    ...
    function display() {
        ...
        if(isset($this->bean->id)){
            ...
            if($this->bean->meeting_type == 'Session'){
                ...
                if (checkDataLockDate($this->bean->date_end) && $this->bean->type_of_class != "Junior") {
                    $html_cover = '<input title="'.$GLOBALS['mod_strings']['LBL_COVER_SESSION'].'" class="button primary" onclick="location.href=\'index.php?module=Meetings&return_module=Meetings&action=EditView&record='.$this->bean->id.'&session_status=Cover\'" type="button" name="CoverSession" value="'.$GLOBALS['mod_strings']['LBL_COVER_SESSION'].'" id="cover_session">';
                    $this->ss->assign('COVER_BUTTON', $html_cover);

                    $html_make_up = '<input title="'.$GLOBALS['mod_strings']['LBL_MAKE_UP_SESSION'].'" class="button primary" onclick="location.href=\'index.php?module=Meetings&return_module=Meetings&action=EditView&record='.$this->bean->id.'&session_status=Make-up\'" type="button" name="MakeUpSession" value="'.$GLOBALS['mod_strings']['LBL_MAKE_UP_SESSION'].'" id="make_up_session">';
                    $this->ss->assign('MAKE_UP_BUTTON', $html_make_up);

                    $html_delete = '<input title="'.$GLOBALS['mod_strings']['LBL_DELETE_SESSION'].'" class="button" onclick="var _form = document.getElementById(\'formDetailView\'); _form.return_module.value=\'Meetings\'; _form.return_action.value=\'ListView\'; _form.action.value=\'Delete\'; if(confirm(\'Are you sure you want to delete this record?\')) SUGAR.ajaxUI.submitForm(_form);" type="button" name="DeleteSession" value="'.$GLOBALS['mod_strings']['LBL_DELETE_SESSION'].'" id="delete_session">';
                    $this->ss->assign('DELETE_BUTTON', $html_delete);
                }
                //Display class
                if($this->bean->type_of_class == "Junior"){
                    ...
                    $class_code = $GLOBALS['db']->getOne("SELECT name FROM c_classes WHERE id = '{$this->bean->class_id}'");
                    $class = '<a href="index.php?module=C_Classes&return_module=C_Classes&action=DetailView&record='.$this->bean->class_id.'" TARGET=_blank>'.$class_code.'</a>';
                }
        ...
        $add_outstanding_template = $this->ss->fetch('custom/modules/J_Class/tpls/oustanding_template.tpl');
        $this->ss->assign('ADD_OUTSTANGDING_TEMPLATE', $add_outstanding_template);
