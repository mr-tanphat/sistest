...
class logicStudent{
    function addCode(&$bean, $event, $arguments){
        ...
        $prefix 	= $part[0];
        $date       = date('ym',strtotime('+ 7hours'. (!empty($bean->date_entered) ? $bean->date_entered : $bean->fetched_row['date_entered'])));
        $date_year          = date('y',strtotime('+ 7hours'. (!empty($bean->date_entered) ? $bean->date_entered : $bean->fetched_row['date_entered'])));
        $code_field = 'contact_id';
        $table 		= $bean->table_name;
        $sep          = '';
        $first_pad    = '0000';
        $padding      = 4;
        $query = "SELECT $code_field FROM $table WHERE ( $code_field <> '' AND $code_field IS NOT NULL) AND id != '{$bean->id}' AND (LEFT($table.$code_field, ".strlen($prefix.$date).") = '".$prefix.$date."') ORDER BY RIGHT($code_field, $padding) DESC LIMIT 1";

        if($row['team_type'] == 'Adult'){    //Bo Sung Student Type
            $ext = 'A';
            $bean->student_type = 'Adult';
        }else{
            $ext = 'Y';
            $bean->student_type = 'Junior';
        }
        //Edit by Lap Nguyen
        //if( empty($bean->fetched_row[$code_field]) ) {
        if( empty($bean->contact_id) ) {
            ...
            if($row = $GLOBALS['db']->fetchByAssoc($result)){
                ...
                $last_code = $prefix . $sep. $date .$ext  . $first_pad;
            }

            $num = substr($last_code, -$padding, $padding);
            $num++;
            $pads = $padding - strlen($num);
            $new_code = $prefix . $sep . $date . $ext ;
    ...
    function handleAddRelationship(&$bean, $event, $arguments){
        if ($arguments['related_module'] =='Contracts'){
            ...
            if( $number_of_student > $arguments['related_bean']->total_student){
                echo '
                <script type="text/javascript">
                alertify.alert(" Lớp đã đủ số lượng học viên !!");
            ...
            if($bean->type == 'Corporate'){
                ...
                if(empty($payment_id)){
                    ...
                    $pm_cop->contacts_j_payment_1contacts_ida = $bean->id;
                    $pm_cop->remain_amount        = $arguments['related_bean']->amount_per_student;
                    $pm_cop->tuition_fee          = $arguments['related_bean']->amount_per_student;
                    $pm_cop->amount_bef_discount  = $arguments['related_bean']->amount_per_student;
                    $pm_cop->total_after_discount = $arguments['related_bean']->amount_per_student;
                    $pm_cop->payment_amount       = $arguments['related_bean']->amount_per_student;
                    ...
                    $pm_cop->tuition_hours        = $arguments['related_bean']->study_duration;
                    $pm_cop->total_hours          = $arguments['related_bean']->study_duration;
                    $pm_cop->remain_hours         = $arguments['related_bean']->study_duration;
                    ...
                    $pm_cop->save();
                    //Create payment Detail Corporate
                    $sql = "SELECT DISTINCT
                    SUM(j_paymentdetail.payment_amount) sum_payment_amount
                    FROM
                    j_paymentdetail
                    INNER JOIN
                    ...
                    AND l1.deleted = 0
                    WHERE
                    (((l1.id = '{$pm_cop->contract_id}')))
                    AND j_paymentdetail.deleted = 0
                    ORDER BY j_paymentdetail.payment_no ASC";
                    $pmd_sum = $GLOBALS['db']->getOne($sql);
                    if($pmd_sum > 0){
                        $pmdd = BeanFactory::newBean('J_PaymentDetail');
                        $pmdd->payment_no        = 1;
                        $pmdd->name              = $arguments['related_bean']->name."-1";
                        $pmdd->before_discount   = format_number($pmd_sum / $arguments['related_bean']->total_student);
                        $pmdd->discount_amount   = 0;
                        $pmdd->sponsor_amount    = 0;
                        $pmdd->status            = "Paid";
                        $pmdd->payment_date      = $timedate->nowDbDate();
                        $pmdd->payment_method    = 'Other';

                        $pmdd->type              = 'Normal';
                        $pmdd->payment_amount    = $pmdd->before_discount;

                        $pmdd->payment_id        = $pm_cop->id;
                        $pmdd->assigned_user_id  = $pm_cop->assigned_user_id;
                        $pmdd->team_id           = $pm_cop->team_id;
                        $pmdd->team_set_id       = $pm_cop->team_id;
    ...
    function handleStudentLogic(&$bean, $event, $arguments){
        ...
        if(isset($_REQUEST['lead_id']) && $_REQUEST['lead_id'] !='') {
            $lead = new Lead();
            $lead->retrieve($_REQUEST['lead_id']);
            if($lead->id) {
                ...
                $GLOBALS['db']->query("INSERT INTO meetings_contacts (id, meeting_id, contact_id, required, accept_status, date_modified, deleted)
                    SELECT id, meeting_id, '{$bean->id}', required, accept_status, date_modified, deleted FROM meetings_leads WHERE lead_id = '{$lead->id}' AND deleted = 0;");
    ...
    function listviewcolor_Stu(&$bean, $event, $arguments) {
        global $current_user;
        $bean->custom_checkbox_class='<input type="checkbox" class="custom_checkbox" module_name="Contacts" onclick="handleCheckBox($(this));" value="'.$bean->id.'"/>';
        $bean->contact_id = '<span class="textbg_blue">'.$bean->contact_id.'</span>';
        if ($_REQUEST['module']=='C_Classes'){
            $class = BeanFactory::getBean('C_Classes',$_REQUEST['record']);

            $bean->checkbox = '<input type="checkbox" class="checkbox_inclass" name="checkbox_inclass" value="'.$bean->id.'">';
        ...
        if ($_REQUEST['module']=='J_Class'){
            $bean->subpanel_button = '<input title="Create Feedback" onclick="window.open(\'index.php?module=J_Feedback&action=EditView&return_module=J_Feedback&return_action=DetailView&return_id=&j_class_j_feedback_1j_class_ida='.$_REQUEST['record'].'&contacts_j_feedback_1contacts_ida='.$bean->id.'&contacts_j_feedback_1_name='.$bean->full_student_name.'\',\'_blank\')" class="button primary" type="submit" name="btn_create_feedback" id="btn_create_feedback" value="Create Feedback">';
        }
        ...
        if ($_REQUEST['module']=='Meetings'){
            ...
            if($meeting->type_of_class != "Junior"){
                ...
                if(is_admin($current_user)){
                    $bean->subpanel_button = '<div style="display: inline-flex;">
                    <input type="button" name="remove_junior_student" class="button remove_junior_student" id="'.$bean->id.'" value="Remove">
                    </div>';
                }
                //else
                //    $bean->subpanel_button = '<label style="color: #E61718;">Locked by date</label>';
    ...
    function autoGenerateAccountPortal(&$bean, $event, $arguments) {
        ...
        if($_POST['module'] == $bean->module_name && $_POST['action'] == 'Save'){
            ...
            if(empty($bean->fetched_row['portal_active']) && $bean->portal_active == '1'){
                $team_type = getTeamType($bean->team_id);
                if($team_type = 'Adult')
                    $user->password = 'apollo123456';
                else $user->password = 'apollojunior2016';
            }
