...
$(document).ready(function() { 
	...
	check_sms_len = function (len) { 
		...
		var sms_len = $("textarea#sms_msg").val().length;
		var messages = Math.ceil(sms_len / 160);
		var remaining = messages * 160 - (sms_len % (messages * 160) || messages * 160);

		$("span#sms_len_notifier").html(remaining + " remaining <span style='float:right;font-weight:bold;'>" +messages+'/3</span>');

		if (sms_len > 480 || sms_len==0) {
