...
require_once("custom/modules/C_Carryforward/runtime_helper_junior.php");
...
for($i = 0; $i < count($row_course); $i++){
    ...
    while($date_run <= $end_run){
        ...
        //Bổ sung số TH Sponsor 100% vào báo cáo #bug 02/05/2016
        $pp_type_list = array(
            0 => 'Enrollment',
            1 => 'Cashholder');
    ...
    if($carry_amount != 0 || $carry_hrs != 0 || abs($beginning_amount) >= 1000 || $collected_amount != 0 || $revenue_amount != 0 || $settle_amount != 0 || $mv_tf_in_amount != 0 || $mv_tf_out_amount != 0 || $cash_in_amount != 0 || $cash_out_amount != 0 || $refund_amount != 0){
        $count++;
        ...
        $html .= "<td valign='TOP' class='oddListRowS1'>{$row_course[$cr_i]['aims_code']}</td>";
        //        else
