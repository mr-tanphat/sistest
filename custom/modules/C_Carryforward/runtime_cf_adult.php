...
require_once("custom/modules/C_Carryforward/runtime_helper_adult.php");
...
for($i = 0; $i < count($row_course); $i++){
    ...
    if(in_array($row_course[$i]['payment_type'], $mv_tf)) {
        ...
        else $mv_tf_in_this_amount =    $row_course[$i]['payment_amount']; 
    }
    ...
    if($carry_amount != 0 || $carry_days != 0 || $beginning_amount != 0 || $collected_amount_this != 0 || $revenue_till_this_amount != 0 ||  $mv_tf_in_this_amount != 0 || $mv_tf_out__this_amount != 0 || $cash_in__this_amount != 0 || $cash_out_this_amount != 0 || $refund_this_amount != 0){
        $count++;
