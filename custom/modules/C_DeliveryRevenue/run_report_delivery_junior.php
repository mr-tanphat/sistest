...
if($this->saved_report->id == '609fd67d-f575-d2e0-3874-57314876faf4'){
   for($i = 0; $i < count($parts); $i++){
        if(strpos($parts[$i], "l6.end_study>=") !== FALSE) $start_date = get_string_between($parts[$i]);
        if(strpos($parts[$i], "l6.end_study<=") !== FALSE) $end_date     = get_string_between($parts[$i]);
        ...
        if(strpos($parts[$i], "l6.date_input='") !== FALSE){
...
calDeliveryJunior($start_date , $end_date , $team_id, $student_id, $class_id, $type);
